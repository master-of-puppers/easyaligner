...
With ASR word-level timestamps in hand, we search for Chapter II's ground-truth text within the ASR transcription. The function `fuzzy_match` handles this in one call. It concatenates all timestamped words from the ASR output, uses [`rapidfuzz`](https://github.com/maxbachmann/RapidFuzz) to find the best matching region, and returns the word indices for the match. 
...
The returned [`FuzzyMatch`](../reference/FuzzyMatch.qmd) object contains the discovered timestamps (`match.start`, `match.end`), as well as the word-level indices. 
...
With approximate start and end times discovered, we run forced alignment using the ground-truth text and the relevant audio region. This step produces precise word-level timestamps based on the original text, while maintaining its formatting, as in [Tutorial 1](tutorial01.qmd).
