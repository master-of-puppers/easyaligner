...
The library supports aligning both from ground-truth transcripts, as well as from ASR (automatic speech transcription) model outputs. Check out the library [`easytranscriber`](https://kb-labb.github.io/easytranscriber/) for an example where `easyaligner` is used as a backend to align ASR outputs. 
