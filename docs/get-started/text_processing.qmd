...
Let's apply some basic normalization to our example from A Tale of Two Cities. To explore the effect of our normalizations, we will import the [`SpanMapNormalizer`](../reference/normalization.SpanMapNormalizer.qmd) class from the `easyaligner` library, and remove punctuation.
...
Unlike `PunktTokenizer`, `paragraph_tokenizer` is a plain callable (i.e. a function). No pre-computation of character spans is needed. Just pass it directly to the `pipeline`:
...
Each alignment segment in the output JSON will then correspond to exactly one paragraph, and paragraph breaks will appear at the correct locations in the [`easysearch`](search.qmd) search interface.
